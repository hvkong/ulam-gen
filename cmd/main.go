...
func main() {
	...
	// If QUICKFOOD_PYROSCOPE_ENDPOINT is set, profiling in push mode will be enabled.
	...
	otlpEndpoint, _ := os.LookupEnv("QUICKFOOD_OTLP_ENDPOINT")
	if otlpEndpoint != "" {
		...
		if envBool("QUICKFOOD_TRUST_CLIENT_TRACEID") {
	...
	// If QUICKFOOD_ENABLE_ALL_SERVICES is either _not set_ or set to a truthy value, all services are enabled. This is the
	// default behavior.
	// If QUICKFOOD_ENABLE_ALL_SERVICES is set to a falsy values, services are opted-in by setting the environment variables
	// below to a truty value.

	if envServe("QUICKFOOD_ENABLE_HTTP_TESTING_SERVICE") {
		server.AddHTTPTesting()
	}

	if envServe("QUICKFOOD_ENABLE_TEST_K6_IO_SERVICE") {
		server.AddTestK6IO()
	}

	if envServe("QUICKFOOD_ENABLE_CONFIG_SERVICE") {
		// Prefix for env vars is QUICKFOOD_CONF_ instead of QUICKFOOD_CONFIG_ to avoid picking up variables
		// generated by K8s from the pod name.
		server.AddConfigHandler(envConfig("QUICKFOOD_CONF_"))
	}

	if envServe("QUICKFOOD_ENABLE_PUBLIC_API_SERVICE") {
		...
		if !envServeAll() {
			server.AddGateway(
				envEndpoint("QUICKFOOD_ENABLE_CATALOG_SERVICE", "QUICKFOOD_CATALOG_ENDPOINT"),
				envEndpoint("QUICKFOOD_ENABLE_COPY_SERVICE", "QUICKFOOD_COPY_ENDPOINT"),
				envEndpoint("QUICKFOOD_ENABLE_WS_SERVICE", "QUICKFOOD_WS_ENDPOINT"),
				envEndpoint("QUICKFOOD_ENABLE_RECOMMENDATIONS_SERVICE", "QUICKFOOD_RECOMMENDATIONS_ENDPOINT"),
				envEndpoint("QUICKFOOD_ENABLE_CONFIG_SERVICE", "QUICKFOOD_CONFIG_ENDPOINT"),
			)
		}
	}

	if envServe("QUICKFOOD_ENABLE_WS_SERVICE") {
		server.AddWebSocket()
	}

	if envServe("QUICKFOOD_ENABLE_CATALOG_SERVICE") {
	...
	if envServe("QUICKFOOD_ENABLE_COPY_SERVICE") {
	...
	// If they are not, URLs are sourced from QUICKFOOD_CATALOG_ENDPOINT and QUICKFOOD_COPY_ENDPOINT.
	if envServe("QUICKFOOD_ENABLE_RECOMMENDATIONS_SERVICE") {
		catalogClient := qphttp.NewCatalogClient(envEndpoint("QUICKFOOD_ENABLE_CATALOG_SERVICE", "QUICKFOOD_CATALOG_ENDPOINT")).WithClient(httpCli)
		copyClient := qphttp.NewCopyClient(envEndpoint("QUICKFOOD_ENABLE_COPY_SERVICE", "QUICKFOOD_COPY_ENDPOINT")).WithClient(httpCli)

		server.AddRecommendations(catalogClient, copyClient)
	}

	if envServe("QUICKFOOD_ENABLE_GRPC_SERVICE") {
...
func clientFromEnv() *http.Client {
	...
	timeout := envDuration("QUICKFOOD_TIMEOUT")
	if timeout == 0 {
		timeout = time.Second
	}
	...
	retriableClient.RetryMax = envInt("QUICKFOOD_RETRIES")

	if retryMin := envDuration("QUICKFOOD_BACKOFF_MIN"); retryMin != 0 {
		retriableClient.RetryWaitMin = retryMin
	}

	if retryMax := envDuration("QUICKFOOD_BACKOFF_MAX"); retryMax != 0 {
...
func envPyroscopeConfig() (pyroscope.Config, bool) {
	pyroscopeAddr, ok := os.LookupEnv("QUICKFOOD_PYROSCOPE_ENDPOINT")
	if !ok {
		return pyroscope.Config{}, false
	}

	svcName, ok := os.LookupEnv("QUICKFOOD_PYROSCOPE_NAME")
	if !ok {
		svcName, ok = os.LookupEnv("QUICKFOOD_OTEL_SERVICE_NAME")
		if !ok {
			svcName = "quickfood"
		}
	}

	svcNamespace, ok := os.LookupEnv("QUICKFOOD_PYROSCOPE_NAMESPACE")
	if !ok {
		svcNamespace = "quickfood"
	}

	svcNamespaceLabel, ok := os.LookupEnv("QUICKFOOD_PYROSCOPE_NAMESPACE_LABEL_NAME")
	if !ok {
		svcNamespaceLabel = "namespace"
	}

	svcGitRef, ok := os.LookupEnv("QUICKFOOD_PYROSCOPE_SERVICE_GIT_REF")
	if !ok {
	...
	return pyroscope.Config{
		...
		BasicAuthUser:     os.Getenv("QUICKFOOD_GRAFANA_CLOUD_USER"),
		BasicAuthPassword: os.Getenv("QUICKFOOD_GRAFANA_CLOUD_PASSWORD"),
...
func envServeAll() bool {
	allSvcs, present := os.LookupEnv("QUICKFOOD_ENABLE_ALL_SERVICES")
	allSvcsB, _ := strconv.ParseBool(allSvcs)

	// If QUICKFOOD_ENABLE_ALL_SERVICES is not defined (default), serve everything.
	if !present {
		return true
	}

	// Otherwise, serve all if QUICKFOOD_ENABLE_ALL_SERVICES is truthy.
	return allSvcsB
}
...
// envDBConnString returns the specified db connection string from QUICKFOOD_DB. It defaults to an in-memory sqlite instance
func envDBConnString() string {
	v, found := os.LookupEnv("QUICKFOOD_DB")
	if !found || v == "" {
