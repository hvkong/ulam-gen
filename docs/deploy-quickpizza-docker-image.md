## Deploy QuickFood Docker image

The [Dockerfile](./Dockerfile) contains the setup for running QuickFood without collecting data with Grafana Alloy.

You can use the Dockerfile or build a Docker image to deploy the QuickFood app on any cloud provider that supports Docker deployments. For simplicity, here are the `Fly.io` instructions:
